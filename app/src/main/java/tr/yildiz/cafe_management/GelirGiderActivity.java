...
public class GelirGiderActivity extends AppCompatActivity implements View.OnClickListener{
    private CalendarView calendarBas;
    private Button urunGoster,gelirGiderGoster,gelirEkle,giderEkle,tamam, iptal;
    private RadioButton radioButtonDay,radioButtonMonth,radioButtonYear;
    private TextView tarih, d_tarih, d_title;
    private EditText edit_title, edit_number;
    ...
    public void init(){
        urunGoster = (Button) findViewById(R.id.urunGoster);
        ...
        urunGoster.setOnClickListener(this);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.urunGoster:
                Log.i("zamann",zaman.toString());
                ...
                if (radioButtonDay.isChecked()){
                    Log.i("gün","seçili");
                    timeControl = "1";
                    Intent intent1 = new Intent(context, SatisRaporuActivity.class);
            ...
            case R.id.gelirGiderGoster:
                ...
                String timeControl1 = "0";
                if (radioButtonDay.isChecked()){
                    Log.i("gün","seçili");
                    timeControl1 = "1";
                    Intent intent1 = new Intent(context, BusinessReportActivity.class);
                    intent1.putExtra("timeControl",timeControl1);
                    intent1.putStringArrayListExtra("time",zaman);
                    startActivity(intent1);
                }
                else if (radioButtonMonth.isChecked()){
    ...
    public void showDialog(String control){
        ...
        tamam.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (control.equals("0")){
                    reference.child("dukkanlar").child(user.getUid()).child("statementExpenditureBusiness").child(yil).child(ay).child(gun).child(edit_title.getText().toString()).setValue(s);
                    Toast.makeText(context,"Gider Eklendi",Toast.LENGTH_SHORT).show();
                }
                else{
                    reference.child("dukkanlar").child(user.getUid()).child("statementIncomeBusiness").child(yil).child(ay).child(gun).child(edit_title.getText().toString()).setValue(s);
